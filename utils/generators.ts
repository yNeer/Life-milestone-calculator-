...
const CATEGORY_COLORS: Record<MilestoneCategory, string> = {
  [MilestoneCategory.Birthday]: '#ef4444',    // Red-500 (Primary Life Events)
  [MilestoneCategory.Anniversary]: '#db2777', // Pink-600
  [MilestoneCategory.Power]: '#2563eb',       // Blue-600
  [MilestoneCategory.Standard]: '#0891b2',    // Cyan-600
  [MilestoneCategory.Weeks]: '#0d9488',       // Teal-600
  [MilestoneCategory.Months]: '#059669',      // Emerald-600
  [MilestoneCategory.Hourly]: '#d97706',      // Amber-600
  [MilestoneCategory.Math]: '#b45309',        // Orange-700 (Distinct from Red)
  [MilestoneCategory.Repdigit]: '#7c3aed',    // Violet-600
  [MilestoneCategory.Sequence]: '#c026d3',    // Fuchsia-600
  [MilestoneCategory.Minutes]: '#4f46e5',     // Indigo-600
  [MilestoneCategory.Seconds]: '#9333ea',     // Purple-600
  [MilestoneCategory.Custom]: '#ca8a04',      // Yellow-600 (Gold)
};
...
export const getCentury = (year: number): string => {
  ...
  const suffix = ["th", "st", "nd", "rd"][century % 10 > 3 ? 0 : Number(century % 100 - century % 10 !== 10) * (century % 10)] || "th";
  return `${century}${suffix} Century`;
};

export const getGoogleCalendarUrl = (milestone: Milestone): string => {
  const start = milestone.date.toISOString().replace(/-|:|\.\d\d\d/g, "");
  const end = addHours(milestone.date, 1).toISOString().replace(/-|:|\.\d\d\d/g, ""); // 1 hour duration
  
  const title = encodeURIComponent(milestone.title);
  const details = encodeURIComponent(`${milestone.description}\n\nGenerated by Life Milestones Calculator`);
...
const generateMath = (limit: number): { value: number, desc: string, subCat: string }[] => {
    ...
    // Squares
    // Removed 144 (12*12) to prevent early timeline clutter
    for (let i = 10; i * i <= limit; i++) {
        if (i % 25 === 0 || i === 100 || i === 50) {
