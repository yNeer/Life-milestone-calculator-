...
const NavBar: React.FC<Props> = ({ currentView, setView, onShareApp }) => {
  ...
  return (
    <>
      ...
      <div className="sticky top-0 z-40 bg-skin-base/80 backdrop-blur-lg border-b border-skin-border/50 px-4 py-3 flex justify-between items-center transition-all duration-300">
         
         {/* Left: Menu & Logo */}
         <div className="flex items-center gap-3">
             <button 
                onClick={() => setIsMenuOpen(!isMenuOpen)}
                className="p-2 rounded-xl hover:bg-skin-input/50 text-skin-text transition-colors border border-transparent hover:border-skin-border/50"
                aria-label="Menu"
             >
         ...
         <div className="flex items-center gap-2">
             <button 
                onClick={onShareApp}
                className="hidden sm:flex items-center gap-2 px-4 py-2 bg-skin-input/60 hover:bg-skin-border/60 rounded-full text-xs font-bold text-skin-text transition-all backdrop-blur-sm border border-skin-border/20"
             >
         ...
         {isMenuOpen && (
             <div className="absolute top-full left-0 m-2 w-64 bg-skin-card/90 backdrop-blur-xl border border-skin-border/50 shadow-2xl rounded-2xl p-2 z-[60] animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden ring-1 ring-black/5">
                 <div className="p-2 mb-2 text-xs font-bold text-skin-muted uppercase tracking-wider border-b border-skin-border/50">
                    Navigation
                 </div>
                 <button onClick={() => handleNavClick('dashboard')} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-skin-input/50 text-left text-sm font-medium text-skin-text transition-colors">
                    <LayoutDashboard size={18} className="text-skin-primary" /> Dashboard
                 </button>
                 <button onClick={() => handleNavClick('visualizations')} className="flex items-center gap-3 w-full p-3 rounded-xl hover:bg-skin-input/50 text-left text-sm font-medium text-skin-text transition-colors">
                 ...
                 <div className="h-px bg-skin-border/50 my-2" />
                 <a 
      ...
      <div className="fixed bottom-6 left-0 right-0 z-50 flex justify-center pointer-events-none px-4">
        <div className="pointer-events-auto bg-skin-card/80 backdrop-blur-2xl border border-white/20 shadow-2xl rounded-full px-2 py-2 flex items-center gap-1 sm:gap-2 max-w-md w-full justify-between sm:justify-center ring-1 ring-black/5">
              ...
              <button
                ...
                className={`flex items-center justify-center gap-2 px-4 sm:px-6 py-3 rounded-full transition-all duration-300 relative overflow-hidden group flex-1 sm:flex-none ${
                  isActive 
                    ...
                    : 'text-skin-muted hover:bg-skin-input/50 hover:text-skin-text'
                }`}
              >
